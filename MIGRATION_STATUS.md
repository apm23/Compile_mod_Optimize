...
| Custom Hotbar Inventory | `custom-hotbar-inventory-1.0.0-tacz-workbench-multipage-fix-mc26.2(1).jar` | `apm23/custom-hotbar-inventory` | `dual/src/main/java/apm23/compilemod/dual/inventory/...` | Input debounce verified run #14; storage verified run #15; payload/codec protocol verified run #16. Server registration, lifecycle, sort/merge conservation logic, and receiver handlers are now migrated without `TaczAmmoCompat`; verification pending. Client UI/mixins and optional TACZ compatibility remain isolated. |
