...
| Custom Hotbar Inventory | `custom-hotbar-inventory-1.0.0-tacz-workbench-multipage-fix-mc26.2(1).jar` | `apm23/custom-hotbar-inventory` | `dual/src/main/java/apm23/compilemod/dual/inventory/...` | Input, storage, payload protocol, server networking/lifecycle, client core, recipe-book mixin, and optional TACZ server/client adapters migrated. Build + dedicated-server smoke without TACZ green through run #22. |
| God Villager | `god-villagers-0.1.0-alpha.85+mc26.2 (4).jar` | `apm23/god-villager` including historical optimizer source | `dual/src/main/resources/data/godvillagers/recipe` + `dual/src/main/java/apm23/compilemod/dual/compat/tacz/godvillager` | Golden recipes migrated. Historical source recovered for Life Steal runtime/mixin + enchant data/tag + JUnit contract; verification pending. Main God Villager baseline entrypoint still originates from historical base JAR rather than source and remains pending. |
...
Golden JAR behavior wins over a newer source HEAD when they disagree. Source repositories, including recoverable historical commits, are implementation references. Every migration must pass compile/regression/smoke evidence before its status can become verified.
