...
| Custom Hotbar Inventory | `custom-hotbar-inventory-1.0.0-tacz-workbench-multipage-fix-mc26.2(1).jar` | `apm23/custom-hotbar-inventory` | `dual/src/main/java/apm23/compilemod/dual/inventory/...` | Input debounce + golden JUnit verified on run #14; server-authoritative 8x27 storage + alternate hotbar + legacy `doubleinv` migration verified by run #15. Payload/codec protocol is now migrated with original `custom_hotbar_inventory:*` wire IDs; receiver registration, client UI, and TACZ compatibility remain isolated pending slices. |
